...
const AntiPattern = () => (
  <SkewTwo fst="linguistic" snd="antipattern"></SkewTwo>
);

export const Intro = () => {
  return (
    <div className="grid grid-cols-3">
      <div className="col-span-3 -skew-x-[10deg] border-dashed border-4 text-slate-900 left-[7px] relative border border-yellow-500">
        <div className="skew-x-[10deg]">
          <Welcome />
        </div>
      </div>
      <div className="col-span-3 sm:col-span-2 lg:col-span-1 bg-stone-300 p-5 border-x-4 border-dashed border-yellow-500">
        <WhatIsIt />
      </div>
      <div className="col-span-3 sm:col-span-1 bg-stone-200 p-5 border-r-4 border-dashed border-yellow-500 lg:border-none">
        <Origin />
      </div>
      <div className="col-span-3 md:col-span-1 bg-stone-100 p-5 border-x-4 border-dashed border-yellow-500">
        <Differ />
      </div>
...
const Origin = () => {
  return (
    <div className="prose max-w-none">
      <h2 className="text-xl 2xl:text-2xl">Origin</h2>
      ...
      <div className="bg-slate-50 p-4 font-bold">
        <div>
          <p className="font-bold prose">
            We are{" "}
            <Skew color="red" className="inline-flex">
...
const IndexPage: React.FC<PageProps<Props>> = ({ data }) => {
  const [selected, setSelected] = React.useState<string>(data.allMdx.nodes[0].id)
      ...
      <div className="flex flex-col items-center pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
          {data.allMdx.nodes.map((node) => (
            ...
            className={twMerge("rounded p-3 bg-white border-4 border-yellow-500 font-bold" , selected === node.id ? "bg-yellow-500" : "hover:bg-slate-100" )} >
              {node.frontmatter.title}
            </button>
          ))}
        </div>
      </div>
      {data.allMdx.nodes.filter(x => x.id === selected).map((node) => (
